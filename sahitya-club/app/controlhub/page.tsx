...
export default function ControlHubPage() {
  ...
  // 🔒 কন্ট্রোল হাবের সিক্রেট গেটওয়ে পাসওয়ার্ড
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (password === "WLC_Control_2026") { // 👈 তোমার কাস্টম পাসওয়ার্ড এখানে সেট করো
  ...
  // 🔄 ফায়ারবেস থেকে pending স্ট্যাটাসের লেখাগুলো পুল করার লজিক
  const fetchPendingWritings = async () => {
    ...
    try {
      ...
      console.error("Error fetching pending data from Firestore:", err);
    } finally {
      setLoading(false);
    }
  };

  // ✅ এক ক্লিকে লেখা Approve করার লজিক (যা মেইন পেজে লাইভ করবে)
  ...
  // 🔐 পাসওয়ার্ড স্ক্রিন (যদি অথেনটিকেটেড না থাকে)
  if (!isAuthenticated) {
    return (
  ...
  // 👑 মেইন কন্ট্রোল হাব ড্যাশবোর্ড UI
  return (
    <main className="min-h-screen bg-[#FAFAFA] text-gray-800 p-6 md:p-12 max-w-5xl mx-auto">
      ...
      <header className="border-b border-gray-200 pb-6 mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button 
          ...
          className="text-xs font-semibold text-stone-500 bg-white border border-gray-200 hover:bg-stone-50 px-4 py-2 rounded-xl transition-colors self-start sm:self-center"
        >
          লগআউট ✕
        </button>
      </header>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
          ...
          <p className="text-gray-500 text-sm font-medium">কোয়েনো পেন্ডিং ডাটা নেই ভাই! অল ক্লিয়ার।</p>
        </div>
      ) : (
            ...
            <div key={post.id} className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row gap-6 justify-between items-start">
              ...
              <div className="flex md:flex-col gap-3 w-full md:w-36 flex-shrink-0">
                <button
                  ...
                  className="flex-1 bg-stone-950 text-white font-bold text-xs py-3 rounded-xl hover:bg-emerald-800 transition-colors shadow-sm disabled:bg-gray-300"
                >
                ...
                <button
                  ...
                  disabled={actionLoading !== null}
                  className="flex-1 bg-white border border-rose-200 text-rose-700 font-bold text-xs py-3 rounded-xl hover:bg-rose-50 transition-colors disabled:bg-gray-300"
                >
                  Reject ✕
                </button>
              </div>
